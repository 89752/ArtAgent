...
def test_color_analysis_url_image_returns_error():
    from src.tools.color_analysis import color_analysis

    with patch(
        "src.tools.image_lookup.lookup_images",
        return_value=[{"title": "T", "image_path": "https://x/1.jpg"}],
    ):
        out = color_analysis.invoke({"title": "T"})
...
# ── museum_search / wiki_lookup（mock _get_json，不联网） ────────
def test_museum_search_mocked():
    ...
    with patch("src.tools.museum_search._get_json", side_effect=fake_get):
...
def test_wiki_lookup_mocked():
    ...
    with patch("src.tools.wiki_lookup._get_json", side_effect=fake_get):
